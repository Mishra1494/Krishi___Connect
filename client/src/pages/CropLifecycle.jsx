...
const CropLifecycle = () => {
  ...
  // --- MOCK DATABASE DATA --- //
  // Moved inside to access `t` for translations
  const CROP_MODELS = useMemo(() => ({
    "Wheat": {
      crop_id: "CROP_WHEAT_01",
      crop_name: t('pages.cropPrediction.wheat', 'Wheat'),
      total_duration_days: 120,
      stages: [
        {
          id: 1,
          stage_name: t('pages.cropLifecycle.stages.wheat.s1.name', 'Germination & Seedling'),
          duration_in_days: 15,
          water_requirements: { level: t('pages.cropLifecycle.level.medium', 'Medium'), value: "30-40%" },
          fertilizer_schedule: t('pages.cropLifecycle.stages.wheat.s1.fertilizer', 'Base application of NPK'),
          pest_disease_risks: [t('pages.cropLifecycle.pests.cutworms', 'Cutworms'), t('pages.cropLifecycle.pests.wireworms', 'Wireworms')],
          required_farmer_actions: [t('pages.cropLifecycle.actions.monitorSoil', 'Monitor soil moisture'), t('pages.cropLifecycle.actions.checkGermination', 'Check for even germination')],
          icon: faSeedling,
          color: "bg-amber-100 text-amber-700 border-amber-300"
        },
        {
          id: 2,
          stage_name: t('pages.cropLifecycle.stages.wheat.s2.name', 'Tillering'),
          duration_in_days: 25,
          water_requirements: { level: t('pages.cropLifecycle.level.high', 'High'), value: "60-70%" },
          fertilizer_schedule: t('pages.cropLifecycle.stages.wheat.s2.fertilizer', 'First top dressing of Nitrogen'),
          pest_disease_risks: [t('pages.cropLifecycle.pests.aphids', 'Aphids'), t('pages.cropLifecycle.pests.powderyMildew', 'Powdery Mildew')],
          required_farmer_actions: [t('pages.cropLifecycle.actions.weedControl', 'Weed control application'), t('pages.cropLifecycle.actions.countTiller', 'Count tiller density')],
          icon: faLeaf,
          color: "bg-lime-100 text-lime-700 border-lime-300"
        },
        {
          id: 3,
          stage_name: t('pages.cropLifecycle.stages.wheat.s3.name', 'Vegetative / Jointing'),
          duration_in_days: 30,
          water_requirements: { level: t('pages.cropLifecycle.level.high', 'High'), value: "70-80%" },
          fertilizer_schedule: t('pages.cropLifecycle.stages.wheat.s3.fertilizer', 'Second top dressing of Nitrogen'),
          pest_disease_risks: [t('pages.cropLifecycle.pests.rust', 'Rust'), t('pages.cropLifecycle.pests.leafBlight', 'Leaf Blight')],
          required_farmer_actions: [t('pages.cropLifecycle.actions.fungicide', 'Fungicide spray if humidity is high'), t('pages.cropLifecycle.actions.monitorCanopy', 'Monitor canopy coverage')],
          icon: faWheatAwn,
          color: "bg-green-100 text-green-700 border-green-300"
        },
        {
          id: 4,
          stage_name: t('pages.cropLifecycle.stages.wheat.s4.name', 'Flowering'),
          duration_in_days: 20,
          water_requirements: { level: t('pages.cropLifecycle.level.veryHigh', 'Very High'), value: "80-90%" },
          fertilizer_schedule: t('pages.cropLifecycle.stages.wheat.s4.fertilizer', 'Foliar spray of Potassium if deficient'),
          pest_disease_risks: [t('pages.cropLifecycle.pests.headBlight', 'Head Blight (Scab)'), t('pages.cropLifecycle.pests.armyworms', 'Armyworms')],
          required_farmer_actions: [t('pages.cropLifecycle.actions.ensureWater', 'CRITICAL: Ensure adequate watering'), t('pages.cropLifecycle.actions.noHerbicides', 'Do not spray herbicides during anthesis')],
          icon: faCloudSunRain,
          color: "bg-yellow-100 text-yellow-700 border-yellow-300"
        },
        {
          id: 5,
          stage_name: t('pages.cropLifecycle.stages.wheat.s5.name', 'Maturation & Harvesting'),
          duration_in_days: 30,
          water_requirements: { level: t('pages.cropLifecycle.level.low', 'Low'), value: t('pages.cropLifecycle.dryDown', 'Dry down') },
          fertilizer_schedule: t('pages.cropLifecycle.none', 'None'),
          pest_disease_risks: [t('pages.cropLifecycle.pests.birds', 'Birds'), t('pages.cropLifecycle.pests.rodents', 'Rodents')],
          required_farmer_actions: [t('pages.cropLifecycle.actions.prepareHarvest', 'Prepare harvesting equipment'), t('pages.cropLifecycle.actions.checkMoisture', 'Check grain moisture content (target < 14%)')],
          icon: faTractor,
    ...
    "Sugarcane": {
      ...
      crop_name: t('pages.cropPrediction.sugarcane', 'Sugarcane'),
      total_duration_days: 360,
      stages: [
        {
          id: 1,
          stage_name: t('pages.cropLifecycle.stages.sugarcane.s1.name', 'Germination'),
          duration_in_days: 45,
          water_requirements: { level: t('pages.cropLifecycle.level.high', 'High'), value: "60%" },
          fertilizer_schedule: t('pages.cropLifecycle.stages.sugarcane.s1.fertilizer', 'Basal dressing: 100% P, 50% K, 33% N'),
          pest_disease_risks: [t('pages.cropLifecycle.pests.termites', 'Termites'), t('pages.cropLifecycle.pests.earlyShootBorer', 'Early Shoot Borer')],
          required_farmer_actions: [t('pages.cropLifecycle.actions.ensureSets', 'Ensure sets are fully covered'), t('pages.cropLifecycle.actions.preEmergence', 'Pre-emergence herbicide')],
          icon: faSeedling,
          color: "bg-amber-100 text-amber-700 border-amber-300"
        },
        {
          id: 2,
          stage_name: t('pages.cropLifecycle.stages.sugarcane.s2.name', 'Tillering (Formative)'),
          duration_in_days: 75,
          water_requirements: { level: t('pages.cropLifecycle.level.veryHigh', 'Very High'), value: "80%" },
          fertilizer_schedule: t('pages.cropLifecycle.stages.sugarcane.s2.fertilizer', 'Top dressing of Nitrogen at 45 & 90 days'),
          pest_disease_risks: [t('pages.cropLifecycle.pests.internodeBorer', 'Internode Borer'), t('pages.cropLifecycle.pests.smut', 'Smut')],
          required_farmer_actions: [t('pages.cropLifecycle.actions.lightIrrigation', 'Frequent light irrigation'), t('pages.cropLifecycle.actions.earthingUp', 'Earthing up operation')],
          icon: faLeaf,
          color: "bg-lime-100 text-lime-700 border-lime-300"
        },
        {
          id: 3,
          stage_name: t('pages.cropLifecycle.stages.sugarcane.s3.name', 'Grand Growth'),
          duration_in_days: 150,
          water_requirements: { level: t('pages.cropLifecycle.level.max', 'Maximum'), value: "100%" },
          fertilizer_schedule: t('pages.cropLifecycle.stages.sugarcane.s3.fertilizer', 'Final Nitrogen application before 120 days'),
          pest_disease_risks: [t('pages.cropLifecycle.pests.redRot', 'Red Rot'), t('pages.cropLifecycle.pests.whitefly', 'Whitefly')],
          required_farmer_actions: [t('pages.cropLifecycle.actions.detrashing', 'Detrashing (removing dry leaves)'), t('pages.cropLifecycle.actions.propping', 'Propping (tying canes) to prevent lodging')],
          icon: faLeaf,
          color: "bg-green-100 text-green-700 border-green-300"
        },
        {
          id: 4,
          stage_name: t('pages.cropLifecycle.stages.sugarcane.s4.name', 'Ripening & Maturity'),
          duration_in_days: 90,
          water_requirements: { level: t('pages.cropLifecycle.level.low', 'Low'), value: t('pages.cropLifecycle.stopIrrigation15', 'Stop 15 days prior') },
          fertilizer_schedule: t('pages.cropLifecycle.none', 'None'),
          pest_disease_risks: [t('pages.cropLifecycle.pests.rats', 'Rats'), t('pages.cropLifecycle.pests.scaleInsects', 'Scale Insects')],
          required_farmer_actions: [t('pages.cropLifecycle.actions.stopIrrigationYield', 'Stop irrigation to increase sucrose content'), t('pages.cropLifecycle.actions.coordMill', 'Coordinate with local sugar mill')],
          icon: faTractor,
          color: "bg-orange-100 text-orange-700 border-orange-300"
        }
      ]
    ...
    <div className="p-4 md:p-8 max-w-7xl mx-auto space-y-6 animate-fade-in relative z-10 font-sans">
        ...
        <div className="space-y-2">
          <h1 className="text-3xl font-extrabold text-slate-800 tracking-tight flex items-center">
            <span className="bg-gradient-to-r from-emerald-600 to-green-500 bg-clip-text text-transparent mr-2">
              {t('pages.cropLifecycle.title', 'Lifecycle Tracker')}
            </span>
            <FontAwesomeIcon icon={faSeedling} className="text-emerald-500 text-2xl" />
          </h1>
          <p className="text-slate-500 font-medium">{t('pages.cropLifecycle.subtitle', 'Dynamically track crop growth stages and smart actions.')}</p>
        </div>
        
        <div className="flex flex-col sm:flex-row gap-4 w-full md:w-auto">
          <div className="flex flex-col gap-1 w-full sm:w-auto flex-1">
            <label className="text-sm font-semibold text-slate-600 uppercase tracking-wider">{t('pages.cropLifecycle.cropSelection', 'Crop Selection')}</label>
            <select 
            ...
            >
              {Object.keys(CROP_MODELS).map(key => (
                <option key={key} value={key}>{t(`pages.cropPrediction.${key.toLowerCase()}`, key)}</option>
              ))}
            </select>
          </div>
          <div className="flex flex-col gap-1 w-full sm:w-auto flex-1">
            <label className="text-sm font-semibold text-slate-600 uppercase tracking-wider">{t('pages.cropLifecycle.sowingDate', 'Sowing Date')}</label>
            <input 
              type="date" 
              value={sowingDate}
      ...
      <div className="bg-white/80 p-6 rounded-3xl border border-white/60 shadow-lg backdrop-blur-md">
        <div className="flex justify-between items-end mb-2">
          <div>
            <h3 className="text-lg font-bold text-slate-800">{t('pages.cropLifecycle.overallProgress', 'Overall Progress')}</h3>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-5 bg-white/80 p-6 md:p-8 rounded-3xl border border-white/60 shadow-xl backdrop-blur-md overflow-hidden relative">
          <h3 className="text-xl font-bold text-slate-800 mb-6 flex items-center">
            ...
            {t('pages.cropLifecycle.developmentStages', 'Development Stages')}
          </h3>
          ...
          <div className="relative pl-6 space-y-8 before:absolute before:inset-0 before:ml-[31px] before:-translate-x-px md:before:mx-auto md:before:translate-x-0 before:h-full before:w-1 before:bg-gradient-to-b before:from-slate-200 before:to-slate-100">
            {enrichedStages.map((stage, idx) => {
              ...
              const statusDisplay = 
                stage.status === 'active' ? t('pages.cropLifecycle.status.active', 'active') :
                stage.status === 'completed' ? t('pages.cropLifecycle.status.completed', 'completed') :
                t('pages.cropLifecycle.status.upcoming', 'upcoming');
                  ...
                  `}>
                    <div className="flex justify-between items-center mb-1">
                      ...
                      <span className="text-xs font-medium text-slate-400">
                        {stage.duration_in_days} {t('pages.yieldPrediction.days', 'Days')}
                      </span>
                    </div>
        ...
        <div className="lg:col-span-7 flex flex-col gap-6">
          ...
          <div className="bg-white/80 p-6 md:p-8 rounded-3xl border border-white/60 shadow-xl backdrop-blur-md">
            <div className="flex items-center gap-4 mb-6 relative">
              ...
              <div>
                <span className="text-sm font-bold text-slate-400 uppercase tracking-wider block mb-1">{t('pages.cropLifecycle.detailedStage', 'Detailed Stage View')}</span>
              ...
              {activeDetailCard.status === 'active' && (
                <div className="absolute top-0 right-0">
                  <div className="bg-emerald-50 border border-emerald-200 text-emerald-700 px-4 py-1.5 rounded-full text-sm font-bold shadow-sm flex items-center gap-2">
                    ...
                    {t('pages.cropLifecycle.currentStage', 'Current Stage')}
                  </div>
                </div>
              )}
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 w-full">
              ...
              <div className="bg-slate-50 p-5 rounded-2xl border border-slate-100">
                <h4 className="text-sm font-bold text-slate-800 uppercase tracking-widest flex items-center gap-2 mb-4">
                  ...
                  {t('pages.cropLifecycle.actionItems', 'Action Items')}
                </h4>
              ...
              <div className="space-y-4">
                <div className="bg-blue-50 p-4 rounded-2xl border border-blue-100 flex flex-col gap-2">
                  <div className="flex items-center justify-between">
                    <h4 className="text-sm font-bold text-blue-900 flex items-center gap-2">
                      <FontAwesomeIcon icon={faDroplet} className="text-blue-500" /> {t('pages.cropLifecycle.waterNeeds', 'Water Needs')}
                    </h4>
                ...
                <div className="bg-teal-50 p-4 rounded-2xl border border-teal-100 flex flex-col gap-2">
                  <h4 className="text-sm font-bold text-teal-900 flex items-center gap-2">
                    <FontAwesomeIcon icon={faVial} className="text-teal-500" /> {t('pages.cropLifecycle.fertilizerPlan', 'Fertilizer Plan')}
                  </h4>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="bg-red-50 p-6 rounded-3xl border border-red-100 shadow-sm relative overflow-hidden">
              ...
              <h4 className="text-sm font-bold text-red-800 uppercase tracking-widest flex items-center gap-2 mb-4 relative z-10">
                ...
                {t('pages.cropLifecycle.pestRisk', 'Pest & Disease Risks')}
              </h4>
            ...
            <div className="bg-gradient-to-br from-indigo-50 to-purple-50 p-6 rounded-3xl border border-indigo-100 shadow-sm relative">
              <h4 className="text-sm font-bold text-indigo-800 uppercase tracking-widest flex items-center gap-2 mb-3">
                ...
                {t('pages.cropLifecycle.weatherIntel', 'Weather Intelligence')}
              </h4>
              <div className="bg-white/60 backdrop-blur-sm rounded-xl p-4 border border-indigo-100/50">
                {activeDetailCard.water_requirements.level === t('pages.cropLifecycle.dryDown', 'Dry down') ? (
                  <div className="flex items-start gap-3">
                    <FontAwesomeIcon icon={faExclamationTriangle} className="text-amber-500 mt-1" />
                    <p className="text-sm font-medium text-slate-700">
                      <strong>{t('pages.cropLifecycle.aiAlert', 'AI Alert')}:</strong> {t('pages.cropLifecycle.rainForecast', 'Light rain forecasted next week. Cover harvested crops immediately to prevent moisture rot.')}
                    </p>
                  </div>
                ) : (
                  <div className="flex items-start gap-3">
                    ...
                    <p className="text-sm font-medium text-slate-700">
                      {t('pages.cropLifecycle.optimalConditions', 'Optimal conditions expected for the next 7 days. Perfect window for fertilizer application.')}
                    </p>
                  </div>
                )}
              </div>
